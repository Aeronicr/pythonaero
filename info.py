...
class Info(commands.Cog, name="Info"):
    ...
    async def правила(self, ctx):
        ...
        embed.add_field(name=":pencil: Загальні положення:",
        ...
        embed.add_field(name=":pencil2: Опис каналів", value="Текстові канали:", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетна-вітальня' — сповіщення про новачків.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'анонси' — сповіщення про стріми.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетні-боти' — команди ботів.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетний-закон' — закон серверу.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетні-правила' — правила серверу.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетна флудильня' — буденне спілкування.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'ганебна сральня' — політичні обговорення.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетні-ігромани' — ігрова балаканина.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетний бордель' — 18+ контент.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетна-ігрова-галерея' — ігрові знятки (скріншоти).", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетна кімната сміху' — меми, приколи, жарти.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетні відясики' — поширення відеороликів.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'клюб' — поширення музики.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетна-історія' — найкращі повідомлення на сервері.", inline=False)
        embed.add_field(name=":small_orange_diamond:", value="'славетний-плей-лист” — стіл замовлень музичному боту.", inline=False)
    ...
    async def on_member_join(self,member):
        for channel in member.guild.channels:
            if str(channel) == "славетна-вітальня":
                ...
                await channel.send(f'{member.guild.name}')
                conn = sqlite3.connect("Aerobot_DB.db")
                cursor = conn.cursor() 
                cursor.execute(f"SELECT guild_id FROM Guilds where guild_id={member.guild.id}")
                if cursor.fetchone()==None:
                    cursor.execute(f"INSERT INTO Guilds VALUES ('{member.guild.id}', '{member.guild.name}')")
                else:
                    pass
                conn.commit()
